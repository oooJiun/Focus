...
class PerceptualLoss:
    def __init__(self, device='cuda'):
        self.device = device
        # Load pre-trained VGG model and move it to the specified device
        self.vgg = vgg19(pretrained=True).features[:36].eval().to(self.device)
        for param in self.vgg.parameters():
            param.requires_grad = False

    def __call__(self, generated_rgb, input_rgb):
        # Normalize input images
        input_rgb = (input_rgb - 0.5) / 0.5
        generated_rgb = (generated_rgb - 0.5) / 0.5

        # Move input tensors to the same device as the model parameters
        input_rgb = input_rgb.to(self.device)
        generated_rgb = generated_rgb.to(self.device)

        # Compute VGG feature maps
        input_features = self.vgg(input_rgb)
        generated_features = self.vgg(generated_rgb)

        # Compute MSE loss between feature maps
        loss = F.mse_loss(generated_features, input_features)
