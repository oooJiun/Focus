...
def train_model(image_dir, mask_dir, clear_dir, blur_dir, epochs, batch_size, learning_rate, model_save_path, result_path):
    ...
    optimizer_G = optim.Adam(list(generator_clear.parameters()) + list(generator_blur.parameters()), lr=learning_rate, betas=(0.5, 0.999))
    optimizer_D_clear = optim.Adam(discriminator_clear.parameters(), lr=learning_rate, betas=(0.5, 0.999))
    optimizer_D_blur = optim.Adam(discriminator_blur.parameters(), lr=learning_rate, betas=(0.5, 0.999))
    ...
    for epoch in range(epochs):
        ...
        for img, mask_img, clear_img, blur_img in tqdm(dataloader):
            ...
            if(loss_cycle_b.item() + loss_cycle_c.item() < 0.15):
                flag = 1

            if(flag and (epoch+1)%1 == 0):
                ...
                loss_G = loss_blur2clear + loss_clear2blur + loss_cycle_c + loss_cycle_b
            else :
                loss_G = loss_cycle_c + loss_cycle_b
            ...
            if(flag and (epoch+1)%1 == 0):
                ...
                epoch_loss_D_clear += loss_D_clear.item()
                epoch_loss_D_blur += loss_D_blur.item()
